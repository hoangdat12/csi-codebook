...
for caseIdx = 1:length(ALL_Case)
    ...
    numPorts = size(frameGrid, 3);   % Số lượng port (4)

    txdata1 = []; 

    for p = 1:numPorts
        % fftshift: đưa DC về giữa NFFT
        txDataF_p = [frameGrid(numRe/2+1:end, :, p); ...
                    zeros(NFFT-numRe, numSymb); ...
                    frameGrid(1:numRe/2, :, p)];
                
        % Điều chế OFDM
        temp_txdata = ofdmModulation(txDataF_p, NFFT);
        
        % Ghép cột dữ liệu mới vào cuối ma trận txdata1
